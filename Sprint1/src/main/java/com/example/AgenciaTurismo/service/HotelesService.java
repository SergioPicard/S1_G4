...
import com.example.AgenciaTurismo.exceptions.ParametrosIncorrectosExcepcion;
...
public class HotelesService implements IHotelesService {
    ...
    public List<HotelAvailableDto> filterHotels(LocalDate dateFrom, LocalDate dateTo, String destination) {
        ...
        if(hotelesRepository.findAll().contains(destination)) {

            for (HotelAvailableDto n : hotelAvailable) {

                boolean dateFromStatus = n.getDisponibleDesde().isAfter(dateFrom);
                boolean dateToStatus = n.getDisponibleHasta().isBefore(dateTo);

                boolean dateEqualFromStatus = n.getDisponibleDesde().isEqual(dateFrom);
                boolean dateEqualToStatus = n.getDisponibleHasta().isEqual(dateTo);

                System.out.println(dateFromStatus);
                System.out.println(dateToStatus);
                System.out.println(dateEqualFromStatus);
                System.out.println(dateEqualToStatus);

                if (!destination.equalsIgnoreCase(n.getLugar())) {
                    throw new ParametrosIncorrectosExcepcion("El destino elegido no existe.");
                }

                if ((!dateFromStatus && !dateToStatus) || (dateEqualFromStatus && dateEqualToStatus)) {
                    throw new ParametrosIncorrectosExcepcion("El hotel no se encuentra disponible en las fechas elegidas.");
                }
            ...
            throw new SinHotelesException("No se reconecen los parametros");
        }
        System.out.println(hotelAvailable);
